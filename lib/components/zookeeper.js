...
var logger = require('pome-logger').getLogger('pome-zookeeper-plugin', __filename);
...
var Component = function(app, opts) {
  ...
  this.path = opts.path || '/pome/servers';
  this.username = opts.username || 'pome';
  this.password = opts.password || 'pome';
...
pro.connectZK = function(cb) {
  ...
  this.client.on('disconnected', function() {
    self.state = STATE.DISCONNECTED;
    logger.error('disconnect with zookeeper server.');
  });

  this.client.on('expired', function() {
    logger.error('zk sessionTimeout');
    if(self.state == STATE.DISCONNECTED) {
      logger.error('begin reconnect');
      self.reconnect();
    }
  });

  this.client.on('authenticationFailed', function() {
    logger.error('zk authenticationFailed.');
  });
  ...
  async.whilst(
    ...
    function (next) {
      ...
      self.connectZK(function(err) {
        if(!!err) {
          logger.error('zk reconnect error:', err);
          setTimeout(
            next,
            count * 1000 * 5
          );
